...
printf("  mount_ubifs\n");
	...
	if (c->empty && (c->ro_mount || c->ro_media)) {
		 ...
		 */
		 printf("  mount_ubifs4  c->ro_mount=%d   c->ro_media=%d\n",c->ro_mount,c->ro_media);
...
printf("  mount_ubifs 22\n");
	...
	if (!c->cbuf) {
		err = -ENOMEM;
		printf("  mount_ubifs11\n");
		goto out_free;
	}

	err = alloc_wbufs(c);
	if (err){
		printf("  mount_ubifs12\n");
		goto out_cbuf;
		}
	...
	if (err){
		printf("  mount_ubifs15\n");
		goto out_lpt;
		}

	err = ubifs_replay_journal(c);
	if (err){
		printf("  mount_ubifs16\n");
		goto out_journal;
		}
	...
	if (err)
		{
		printf("  mount_ubifs19\n");
		goto out_infos;
		}

	err = dbg_debugfs_init_fs(c);
	if (err){
		printf("  mount_ubifs20\n");
		goto out_infos;
		}
