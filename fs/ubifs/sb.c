	...
	if (c->fmt_version > UBIFS_FORMAT_VERSION) {
		...
		if (!c->ro_mount ||
			...
			if (c->ro_compat_version <= UBIFS_RO_COMPAT_VERSION) {
				...
				printf("  ubifs_read_superblock3\n");
				err = -EROFS;
			} else
			printf("  ubifs_read_superblock4\n");
				err = -EINVAL;
			goto out;
		}
