...
static void nand_set_defaults(struct nand_chip *chip, int busw)
{
	...
	{
		debug("nand set nand_command\n");
		chip->cmdfunc = nand_command;
	}else{
		debug("chip->cmdfunc =%x\n",chip->cmdfunc);
		debug(" cmdfunc  not null\n");
	}
						  ...
						  struct nand_flash_dev *type)
{
	...
	int i, maf_idx;
	u8 id_data[8];
debug(" nand_get_flash_type 1\n");
	/* Select the device */
	chip->select_chip(mtd, 0);
debug(" nand_get_flash_type 2\n");
	/*
	...
	if(!chip->cmdfunc)
		debug(" cmdfunc == null\n");
	chip->cmdfunc(mtd, NAND_CMD_RESET, -1, -1);
debug(" nand_get_flash_type 2-1\n");
	/* Send the command for reading device ID */
	chip->cmdfunc(mtd, NAND_CMD_READID, 0x00, -1);
debug(" nand_get_flash_type 2-2\n");
	/* Read manufacturer and device IDs */
	*maf_id = chip->read_byte(mtd);
	*dev_id = chip->read_byte(mtd);
debug(" nand_get_flash_type 3\n");
	/*
		    ...
		    struct nand_flash_dev *table)
{
	...
	struct nand_flash_dev *type;
	int ret;
debug("  nand_scan_ident 1\n");
	if (chip->flash_node) {
		ret = nand_dt_init(mtd, chip, chip->flash_node);
		if (ret)
			return ret;
	}
debug("  nand_scan_ident 2\n");
	/* Set the default functions */
	nand_set_defaults(chip, chip->options & NAND_BUSWIDTH_16);
debug("  nand_scan_ident 3\n");
	/* Read the flash type */
	type = nand_get_flash_type(mtd, chip, &nand_maf_id,
				   &nand_dev_id, table);
debug("  nand_scan_ident 4\n");
	if (IS_ERR(type)) {
