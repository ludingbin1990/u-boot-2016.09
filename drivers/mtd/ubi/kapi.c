...
struct ubi_volume_desc *ubi_open_volume(int ubi_num, int vol_id, int mode)
{
	...
	printf("open device %d, volume %d, mode %d", ubi_num, vol_id, mode);
	...
	if (!ubi)
		{
		printf("  ubi_open_volume2\n");
		return ERR_PTR(-ENODEV);
		}

	if (vol_id < 0 || vol_id >= ubi->vtbl_slots) {
		printf("  ubi_open_volume3\n");
		err = -EINVAL;
		goto out_put_ubi;
	}
					   ...
					   int mode)
{
	...
	printf("open device %d, volume %s, mode %d", ubi_num, name, mode);
printf("ubi_open_volume_nm \n");
	if (!name)
		return ERR_PTR(-EINVAL);
	...
	ubi = ubi_get_device(ubi_num);
	if (!ubi){
		printf(" ubi_get_device  failed \n");
		return ERR_PTR(-ENODEV);
		}
	spin_lock(&ubi->volumes_lock);
	/* Walk all volumes of this UBI device */
	printf("ubi->vtbl_slots=%d \n",ubi->vtbl_slots);
